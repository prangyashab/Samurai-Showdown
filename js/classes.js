...
class Fighter extends Sprite {
  ...
  update() {
    ...
    // gravity function
    // Use groundLevel if defined, otherwise fallback
    const ground = typeof groundLevel !== 'undefined' ? groundLevel : canvas.height - 140

    if (this.position.y + this.height + this.velocity.y >= ground) {
      this.velocity.y = 0
      this.position.y = ground - this.height
      this.jumps = 2 // Reset jumps when touching ground
  ...
  draw() {
    ...
    // Pivot at center of hitbox
    const pivotX = this.position.x + this.width / 2

    c.translate(pivotX, this.position.y)

    if (!this.spritesFlipped) {
      // Standard sprite (faces right)
      if (this.facing === 'left') {
        c.scale(-1, 1)
      }
    } else {
      // Flipped sprite (faces left)
